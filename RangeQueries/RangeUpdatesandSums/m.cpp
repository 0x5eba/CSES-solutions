...
void setpropaga(int inx, int cnt_num){
    if (lazy[inx]){
        tree[inx] = lazy[inx]*cnt_num;
...
void setvalues(int tl, int tr, ll val,int l = 0, int r = N, int inx = 1){
    setpropaga(inx,r-l);
    // tutto fuori
    if (tr <= l || tl >= r || r <= l)
        return;
    ...
    if (tl <= l && tr >= r){
        lazy[inx] = val;
        setpropaga(inx,cnt_num);
        return;
    }
    setpropaga(inx, cnt_num);
    int mid = (l+r)/2;
    setvalues(tl,tr,val,l,mid,inx*2);
    setvalues(tl,tr,val,mid,r,inx*2+1);
    tree[inx] = tree[inx*2]+tree[inx*2+1];
}
